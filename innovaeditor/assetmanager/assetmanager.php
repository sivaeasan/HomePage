<?php 
$ca_db_filename = '../../documents/centraladmin.ezg.php';
...
{
	global $ca_db_filename;
		$auth=false; 
		$user_account=array(); 
	if(file_exists($ca_db_filename)) 
	{
        $fp=fopen($ca_db_filename,'r');
        $fsize=filesize($ca_db_filename);
        if($fsize > 0)
		{
	...
	if(isset($users) && $users!='') 
	{
		...
		if(!empty($users_arr)) 
		{
			foreach($users_arr as $k=>$v) { if(in_array($username,$v)) {$user_account=$v; break;} }
		}
	}
	if(!empty($user_account))
	{
		...
		{
			foreach($user_account['access'] as $k=>$v)  { if($v['type']=='1') {$auth=true; break;} }
		}
		else { if($user_account['access'][0]['type']=='1')	{$auth=true;}  }
    }
    return $auth;
}
function format_users_on_read($users_p) 
{
	...
	{	
		...
		{
			$access_str=GFS($access,'<access id="'.$j.'" ','>');
			list($section,$type)=explode (' ',$access_str);
			$access_arr []=array(substr($section,0,strpos($section,'='))=>GFS($section,'="','"'), substr($type,0,strpos($type,'='))=> GFS($type,'="','"'));
			$j++;
		}	
		$users_array []=array('id'=>$i, 'username'=>GFS($username,'="','"'), 'password'=>GFS($password,'="','"'), 'access'=>$access_arr);		
...
$oep_pass_mode = (isset($_SESSION['page_id']) && isset($_SESSION['cur_pwd'.$_SESSION['page_id']])? true: false);
$pass_mode = (isset($_SESSION['page_id']) && isset($_SESSION['admin'.$_SESSION['page_id']])? true: false);
if(!isset($_SESSION['SID_ADMIN']))
{
	if(!isset($_SESSION['cur_user']))		{ if(!$pass_mode && !$oep_pass_mode) { echo "Not allowed!"; exit;} } 
	elseif(checkauth($_SESSION['cur_user'])==false)   { echo "Not allowed!"; exit; }		
}
?>
<?php
...
$AllowedTypes = "gif|jpg|png|mp3|swf|asf|avi|mpg|mpeg|wav|wma|mid|wmw|mov|ram|bmp|pdf";
...
function isTypeAllowed($sFileName)
  ...
  if($AllowedTypes=="*") return true;
    ...
    if(( ereg($AllowedTypes,getExt($sFileName)) ) && (substr_count($sFileName,'.')==1))
    return true;
  else
    return false;
  }
...
function selectFile(index)
  {
	sFile_RelativePath = '../assets' + document.getElementById("inpFile"+index).value;
  ...
  if(bReturnAbsolute)
	...
	document.getElementById("inpSource").value= pDir+'innovaeditor/assets' + document.getElementById("inpFile"+index).value;
